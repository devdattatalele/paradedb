...
impl CustomScan for JoinScan {
    ...
    fn shutdown_custom_scan(state: &mut CustomScanStateWrapper<Self>) {
        ...
        // out of this hook; a release placed after it would never run, leaving the senders to
        // `release_control_senders_on_detach`, which clears them at abort while the mapping is
        // still live. The query is done producing here, so the senders aren't needed, and the
        // drop runs while DSM is mapped.
